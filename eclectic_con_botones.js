/**
 * ECLECTIC - Sistema con Botones
 * 
 * FUNCIONES INCLUIDAS:
 * - crearBotones() - Crea botones en la hoja
 * - actualizarEclecTRIM() - Función principal
 * - diagnosticarEclec() - Función de diagnóstico
 * - actualizarEclecTRIM_sinUI() - Versión sin UI para debugging
 */

/**
 * Crea botones en la hoja para ejecutar las funciones
 */
function crearBotones() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sh = ss.getActiveSheet();
  
  // Agregar texto explicativo primero
  sh.getRange('A1').setValue('🎯 ECLECTIC - Sistema de Golf');
  sh.getRange('A2').setValue('🔍 Para diagnosticar: ejecutar diagnosticarEclec');
  sh.getRange('A3').setValue('✅ Para actualizar: ejecutar actualizarEclecTRIM');
  sh.getRange('A4').setValue('💡 Ve a Extensiones → Apps Script para ejecutar las funciones');
  
  SpreadsheetApp.getUi().alert('✅ Texto explicativo creado exitosamente!\n\nPara ejecutar las funciones:\n1. Ve a Extensiones → Apps Script\n2. Selecciona la función que quieras ejecutar\n3. Haz clic en Ejecutar');
}
...
/**
 * Función para ejecutar desde el editor (sin UI) - Para debugging
 */
function actualizarEclecTRIM_sinUI() {
  const ss  = SpreadsheetApp.getActiveSpreadsheet();
  const shS = ss.getSheetByName('Semanas');
  const shE = ss.getSheetByName('ECLEC');
  const shP = ss.getSheetByName('PAR CAMPO');
  ...
  /*─── VALIDAR ───*/
  if (!shS || !shE || !shP || !shT) {
    console.log('⚠️ Falta alguna hoja requerida.');
    return;
  }
  ...
  if ([colGHIN_S, colGHIN_E, colGHIN_T, colTRIM_T].some(c=>c===-1)) {
    console.log('⚠️ Faltan columnas GHIN o TRIMESTRE.');
    return;
  }
